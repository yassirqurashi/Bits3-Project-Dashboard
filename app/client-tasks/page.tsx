...
const styles = `
  ...
  .ct-top { display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 22px; }
  ...
  .ct-btn-light { color: #151236; background: #fff; border: 1px solid rgba(230,230,240,0.95); box-shadow: 0 10px 24px rgba(21,18,54,0.06); }
  .ct-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 14px; margin-bottom: 22px; }
  .ct-stat { background: rgba(255,255,255,0.94); border: 1px solid rgba(230,230,240,0.9); border-radius: 22px; padding: 18px; box-shadow: 0 14px 38px rgba(21,18,54,0.06); position: relative; overflow: hidden; }
  .ct-stat::after { content: ''; position: absolute; width: 86px; height: 86px; right: -26px; top: -28px; border-radius: 26px; background: var(--client-primary-soft); transform: rotate(18deg); }
  ...
  .ct-value { color: #151236; font-size: 28px; line-height: 1; font-weight: 900; margin-top: 12px; position: relative; z-index: 1; }
  ...
  .ct-panel-head { padding: 18px 20px; border-bottom: 1px solid #f0eff6; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
  .ct-panel-title { font-size: 15px; font-weight: 900; }
  .ct-list { padding: 14px; display: grid; gap: 12px; }
  .ct-card { border: 1px solid #eeedf6; border-radius: 18px; background: #fff; padding: 16px; display: grid; gap: 12px; transition: transform 0.24s ease, box-shadow 0.24s ease, border-color 0.24s ease; }
  .ct-card:hover { transform: translateY(-4px); border-color: var(--client-primary); box-shadow: 0 18px 42px rgba(21,18,54,0.10); }
  ...
  .ct-pill { display: inline-flex; border-radius: 999px; padding: 6px 10px; font-size: 11px; font-weight: 900; color: var(--client-primary); background: var(--client-primary-soft); white-space: nowrap; }
...
export default function ClientTasksPage() {
      ...
      <main style={{
        ...
        <div style={{
          ...
          <div style={{ marginTop: 22, height: 7, borderRadius: 999, background: isDark ? 'rgba(255,255,255,0.10)' : `${primary}18`, overflow: 'hidden' }}>
            <div style={{ width: '55%', height: '100%', borderRadius: 999, background: primary, animation: 'clientTasksLoaderBar 1.25s ease-in-out infinite' }} />
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main className={`ct-root ${theme === 'dark' ? 'dark' : ''}`} style={{ '--client-primary': primary, '--client-primary-soft': `${primary}14` } as CSSProperties}>
      <style>{styles}</style>
      <div className="ct-shell">
